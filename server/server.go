...
import (
	...
	"github.com/hhzhhzhhz/local-dns/cache"
...
func New(backend Backend, db backends.Storage, config *Config) *Server {
	return &Server{
		backend: backend,
		config:  config,
		db: db,
...
var (
	RequestTotal = "dns_request_total"
)

// todo
func (s *Server) dnsRequestTotal(w dns.ResponseWriter, req *dns.Msg, resp *dns.Msg) {
	s.mux.Lock()
	defer s.mux.Unlock()
	...
	for _, v := range re.Kvs {
		if err := json.Unmarshal(v.Value, item); err != nil {
			continue
		}
	}
	if item.Domain == "" {
...
func (s *Server) ServeDNS(w dns.ResponseWriter, req *dns.Msg) {
	...
	defer func() {
		request_log(w, req, rd)
		s.dnsRequestTotal(w, req, rd)
	}()
	...
	// if the special ns for subdomains
	...
	defer func() {
		...
		// ISet TTL to the minimum of the RRset and dedup the message, i.e. remove identical RRs.
		m = s.dedup(m)
	...
	if len(m.Answer) > 0 {
		for _, as := range m.Answer {
			switch as.(type) {
			case *dns.NS:
				r, err := exchangeWithRetry(s.dnsUDPclient, req, as.(*dns.NS).Ns)
				if err == nil {
					r.Compress = true
					r.Id = req.Id
					m = r
...
//func (s *Server) WrapperARs(q dns.Question, name string, previousRecords []dns.RR, bufsize uint16, dnssec, both bool) (records []dns.RR, err error) {
//	return s.AddressRecords(q, name, previousRecords, bufsize, dnssec, both)
//}
...
func (s *Server) AddressRecords(q dns.Question, name string, previousRecords []dns.RR, bufsize uint16, dnssec, both bool) (records []dns.RR, err error) {
	...
	for _, serv := range services {
		...
		switch {
		case strings.ToLower(serv.Text) == NS:{
...
func isEtcdNameError(err error, s *Server) bool {
	...
	if err != nil {
		log.Logger().Error("error from backend: %s", err)
	}
	return false
}
